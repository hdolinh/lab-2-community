...
**Answer:** The `agnes()` function and the `hclust()` funcation are very similar and both 

creates a dendrogram from the bottom up, meaning it builds upon similar paired clusters. Where as `hclust()` joins the two most similar clusters until there is a single one. 

distance between observations 

Both methods start at the bottom up

very similar, heirarchai bottom up
agnes gives the agglomerative coefficient which is useufl in describing the strengths of the clustering structure
...
In your own words how does agnes() differ from diana()? See HOMLR 21.3.1 Agglomerative hierarchical clustering, slides from Lecture 05. Clustering and help documentation (?agnes(), ?diana()).

**Answer:** `agnes()` differs from `diana()` because the `diana()` function builds a dendrogram from the top down where it creates clusters based on disimiliarites and so we are also given a divisive coefficient.
...
The silhouette method found the optimal number of clusters is three and the gap statistic method found the optimal number of clusters is four. This tells us there is not necessarily a definitively clear number of optimal clusters, but 3-4 clusters would be sufficient. 

### Question

In dendrogram plots, which is the biggest determinant of relatedness between observations: the distance between observations along the labeled axes or the height of their shared connection? See HOMLR 21.5 Working with dendrograms.

**Answer:** In dendrogram plots, the relatedness between observations is most determined by the height of their shared connection. This is because height is the measure of distance, the distance on the axes don't have a connection to measure distance. It shows which observations are clustered together. 
...
Why is the pca_method of “GramSVD” chosen over “GLRM”? See HOMLR 17.4 Performing PCA in R.
...
**Answer:** The category carrot contributes the least to PC1 but postitively toward PC2.
...
**Answer:** In ecology, unconstrained ordination is used to visualize multivariate data. Unconstrained ordination allows us to test 
...
What sites are most differentiated by CCA1, i.e. furthest apart along its axis, based on species composition AND the environmnent? What is the strongest environmental vector for CCA1, i.e. longest environmental vector in the direction of the CCA1 axes?
